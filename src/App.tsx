...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#07090e] text-slate-100 flex flex-col justify-between selection:bg-emerald-500 selection:text-black font-sans antialiased relative overflow-x-hidden">
      {/* Halos d'ambiance */}
      <div className="absolute top-0 left-1/4 w-[600px] h-[400px] bg-indigo-600/10 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute top-20 right-1/4 w-[500px] h-[500px] bg-emerald-500/10 rounded-full blur-[150px] pointer-events-none" />

      {/* Navigation */}
      <header className="border-b border-slate-800/80 bg-slate-950/70 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-emerald-500 to-indigo-600 flex items-center justify-center font-black text-white shadow-lg shadow-emerald-500/20">
              <Zap className="w-5 h-5 text-white" />
            </div>
            <div>
              <div className="font-extrabold text-lg tracking-tight flex items-center gap-1.5">
                Audit<span className="text-emerald-400">IA</span>
                <span className="text-[10px] px-2 py-0.5 rounded-md bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 font-mono">PRO</span>
              </div>
              <p className="text-[10px] text-slate-400 hidden sm:block">Diagnostic heuristique de conversion</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-10 flex-1 w-full space-y-16">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-left">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-xs font-semibold text-emerald-300">
              <Sparkles className="w-3.5 h-3.5" />
              Déconstruction en 3 couches ergonomiques
            </div>
            ...
            <p className="text-slate-400 text-sm sm:text-base max-w-xl leading-relaxed">
              Analysez la clarté immédiate, les leviers de rassurance et la friction cognitive de votre page grâce à notre découpage en couches d'optimisation.
            </p>

            {/* Formulaire */}
            <form onSubmit={(e) => runAudit(e)} className="p-3 sm:p-4 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-2xl backdrop-blur-md space-y-3">
              <div className="flex flex-col sm:flex-row gap-2.5">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3.5 rounded-2xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-emerald-500 transition-colors"
                  />
                </div>
                <select
                  value={niche}
                  onChange={(e) => setNiche(e.target.value)}
                  className="py-3.5 px-4 rounded-2xl bg-slate-950 border border-slate-800 text-slate-200 text-sm focus:outline-none focus:border-emerald-500 transition-colors"
                >
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600 hover:opacity-95 active:scale-[0.99] disabled:opacity-50 text-white font-bold text-sm shadow-xl shadow-emerald-600/20 flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
                {loading ? (
                  <>
                    <Clock className="w-5 h-5 animate-spin" />
                    <span>Décomposition heuristique des tranches...</span>
                  </>
                ) : (
                  <>
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400 pt-1">
              <span>Tester un scénario type :</span>
              <button
                type="button"
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 font-medium flex items-center gap-1 transition-colors cursor-pointer"
              >
                Studio Web <ChevronRight className="w-3.5 h-3.5 text-slate-500" />
              </button>
              <button
                type="button"
                ...
                className="px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-300 font-medium flex items-center gap-1 transition-colors cursor-pointer"
              >
                E-commerce <ChevronRight className="w-3.5 h-3.5 text-slate-500" />
              </button>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-col items-center justify-center relative">
            <div className="relative group w-full max-w-[440px]">
              <div className="relative rounded-3xl overflow-hidden border border-slate-800/80 bg-slate-950/60 shadow-2xl backdrop-blur-md">
                <img
                  src="https://i.postimg.cc/8CMTqDwj/hero-3d.png"
        ...
        {result && (
          <section className="space-y-6 animate-in fade-in duration-500">
            <div className="p-6 sm:p-8 rounded-3xl bg-slate-900 border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
              <div className="space-y-2 text-left">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-mono uppercase px-2.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                    {result.nicheLabel}
                  </span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800">
                <span className="text-xs font-medium text-slate-400 flex items-center gap-1.5">
                  <FileSearch className="w-4 h-4 text-emerald-400" /> Clarté Immédiate
                </span>
                <div className="text-2xl font-black text-white mt-2">{result.metrics.clarity}%</div>
                <div className="w-full bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${result.metrics.clarity}%` }} />
                </div>
              </div>

              <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800">
                <span className="text-xs font-medium text-slate-400 flex items-center gap-1.5">
                  <Sparkles className="w-4 h-4 text-cyan-400" /> Force de l'Accroche
                </span>
                <div className="text-2xl font-black text-white mt-2">{result.metrics.hook}%</div>
                <div className="w-full bg-slate-800 h-1.5 rounded-full mt-2 overflow-hidden">
                  <div className="bg-cyan-500 h-full rounded-full" style={{ width: `${result.metrics.hook}%` }} />
                </div>
              </div>

              <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800">
                <span className="text-xs font-medium text-slate-400 flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4 text-rose-400" /> Signaux de Rassurance
                </span>
            ...
            {/* Plan d'action */}
            <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950/30 border border-slate-800 space-y-4 shadow-xl">
              <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm">
                <Target className="w-5 h-5" />
                <span>3 Actions prioritaires à déployer</span>
              </div>
              <div className="space-y-3">
                {result.recommendations.map((rec, idx) => (
                  <div key={idx} className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 text-sm text-slate-200 flex items-center gap-4">
                    <span className="w-8 h-8 rounded-xl bg-emerald-600/20 border border-emerald-500/30 text-emerald-400 flex items-center justify-center font-bold text-xs shrink-0">
                      #{idx + 1}
                    </span>
          ...
          <section className="pt-6 border-t border-slate-900 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800/80 space-y-2">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Vérification du 'Scroll 0' : un visiteur pressé doit immédiatement comprendre votre métier, votre secteur géographique et son bénéfice direct.
              </p>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-slate-900/40 border border-slate-800/80 space-y-2">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Détection des signaux d'autorité : labels professionnels, garanties contractuelles, avis vérifiés et visages humains pour lever la méfiance.
              </p>
            </div>
